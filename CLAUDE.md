...
- **Data layer (`data/`):** `model/` (`Receipt`, `ReceiptItem`, `ReceiptDraft`); `ocr/MlKitTextRecognizer`; `parse/` — the `ReceiptParser` interface with `GeminiReceiptParser` (Firebase AI Logic, model `gemini-2.5-flash`), `RegexReceiptParser` (offline fallback), and `ReceiptParserRouter` (tries Gemini, falls back on any exception). `ReceiptJsonMapper` maps Gemini JSON → draft and is pure/JVM-testable. `auth/AuthManager` (anonymous sign-in); `repository/` — `ReceiptRepository` interface + `FirestoreReceiptRepository` writing to `users/{uid}/receipts`.
