 ...
 *   display time via [pendingRefund] (per-card [ReceiptCard.returnBase] × currentShare), so changing
 *   the configured share — or a per-item devolución selection — is reflected live.
...
/** Total amount to be returned across the pending receipts at the given [share]. */
fun ReceiptSummary.pendingRefund(share: Double): Double =
    round2(active.sumOf { it.returnAmount(share) })
